...
let candidates = [
  {
    ...
    name: "Daniel John",
    party: "ABC",
    details: "Running for President",
  },
  {
    id: "candidate-2",
    name: "Emmanuel Kingsley",
    party: "XYZ",
    details: "Running for President",
  },
  {
    ...
    name: "Jerry Musa",
    party: "ABC",
    details: "Running for Vice President",
  },
  {
    id: "candidate-4",
    name: "Buhari Tinubu",
  ...
  },
  {
    ...
    name: "Ebele Nelson",
    party: "ABC",
    details: "Running for Senate President",
  },
  {
    id: "candidate-6",
...
app.post("/generate-challenge", (req, res) => {
  ...
  const challenge = base64url(
    Buffer.from(new Uint8Array(32).map(() => Math.floor(Math.random() * 256)))
  );
  console.log('Generated challenge:', challenge); // Log the generated challenge

  // Store the challenge temporarily for verification during the enrollment process
  challenge[voterId] = challenge;
  console.log('Stored challenge for voter:', voterId); // Log storage action
...
app.post("/register-fingerprint", async (req, res) => {
  ...
  try {
    const result = await fido.attestationResult(attestation, {
      challenge: challenges[userId], // Use the stored challenge for verification
    });
    ...
    res
      .status(400)
      .json({ success: false, message: "Fingerprint registration failed." });
  }
});
...
app.post("/biometric-vote", async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({
      success: false,
      message: "Error during fingerprint authentication.",
    });
  }
});
